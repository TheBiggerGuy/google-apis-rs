...
extern crate mime;
extern crate strsim;
extern crate google_alertcenter1_beta1 as api;
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::AlertCenter<hyper::Client, Authenticator<DefaultAuthenticatorDelegate, JsonTokenStorage, hyper::Client>>,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    ...
    fn _alerts_feedback_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _alerts_feedback_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    fn _alerts_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["order-by", "page-token", "filter", "page-size", "customer-id"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
            ...
            ("alerts", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("feedback-create", Some(opt)) => {
                        call_result = self._alerts_feedback_create(opt, dry_run, &mut err);
                    },
                    ("feedback-list", Some(opt)) => {
                        call_result = self._alerts_feedback_list(opt, dry_run, &mut err);
                    },
                    ("get", Some(opt)) => {
                        call_result = self._alerts_get(opt, dry_run, &mut err);
                    },
                    ("list", Some(opt)) => {
    ...
    fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match cmn::application_secret_from_directory(&config_dir, "alertcenter1-beta1-secret.json",
        ...
        let auth = Authenticator::new(  &secret, DefaultAuthenticatorDelegate,
                                        ...
                                        JsonTokenStorage {
                                          program_name: "alertcenter1-beta1",
